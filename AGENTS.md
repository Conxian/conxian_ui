...
*   **Base Canvas (60%)**: Enforce an "Ivory" or warm off-white foundation (e.g., #FDFBF7 or #F9F8F6) for all primary application backgrounds.
*   **Surface Layers (30%)**: Use pure white (#FFFFFF) for data cards, modals, and internal sections. Rely on structural spacing and micro-borders rather than heavy drop shadows to create depth.
...
*   **Data Legibility**: Financial data, yields, and analytics must use stark, high-contrast dark typography (text-text / #333333) against the light background.
...
*   **Palette**: strictly apply the 60-30-10 Ivory-led palette.
*   **Contrast**: Ensure all text contrast ratios meet strict institutional accessibility standards (WCAG AAA for financial data).
*   **Components**: Use canonical UI components (Button, Input, Card, Badge, Table) to maintain theme consistency.
